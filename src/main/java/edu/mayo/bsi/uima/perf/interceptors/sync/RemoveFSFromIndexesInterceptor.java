...
/**
 * Intercepts {@link org.apache.uima.cas.FSIndexRepository#removeFS(FeatureStructure)} calls to keep
    ...
    public static void intercept(@SuperCall Runnable call, @This FSIndexRepository indexRepository, @Argument(0) FeatureStructure fs) {
        try {
            if (fs instanceof Annotation) {
                AnnotationIndex index = AnnotationIndices.getForFSIndexRepository(indexRepository, false);
                if (index != null) {
